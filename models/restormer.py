...
def to_3d(x):
    # x: [b, c, h, w]
    b, c, h, w = x.size()
    # permute -> [b, h, w, c], then flatten and return [b, h*w, c]
...
def to_4d(x, h, w):
    # x: [b, h*w, c]
    b, hw, c = x.size()
    # reshape to [b, h, w, c] then permute back to [b, c, h, w]
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q.view(b, head, c, h * w)
        k = k.view(b, head, c, h * w)
        v = v.view(b, head, c, h * w)
        q = F.normalize(q, dim=-1)
        k = F.normalize(k, dim=-1)
        attn = (q @ k.transpose(-2, -1)) * self.temperature
        attn = attn.softmax(dim=-1)
        out = attn @ v
        out = out.view(b, head * c, h, w)
...
class Restormer(nn.Module):
    def __init__(
        ...
        dual_pixel_task=False,  # True for dual-pixel defocus deblurring
    ):
